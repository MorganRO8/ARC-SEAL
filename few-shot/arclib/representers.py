...
from utils.np_cache import np_lru_cache
...
class RotatedGridRepresenter(GridRepresenter):
    """Render a grid after rotating it by 90 degrees clockwise."""

    def __init__(
        self,
        base: Optional[GridRepresenter] = None,
        *,
        header: str = "Rotated 90° clockwise view:",
    ):
        self.base = base or DelimitedGridRepresenter()
        self.header = header

    def encode(self, grid: Grid, **_: object) -> str:
        rotated = np.rot90(grid, k=-1)
        body = self.base.encode(rotated)
        return f"{self.header}\n{body}"

    def decode(self, encoded_str: str, **_: object) -> Grid:
        body = encoded_str
        if self.header and encoded_str.startswith(self.header):
            body = encoded_str[len(self.header) :].lstrip("\n")
        rotated = self.base.decode(body)
        return np.rot90(rotated, k=1)

    def __repr__(self) -> str:
        return f"RotatedGridRepresenter(base={self.base!r}, header={self.header!r})"
...
class DiagonalSliceRepresenter(GridRepresenter):
    ...
    def decode(self, encoded_str: str, **_: object) -> Grid:
        raise NotImplementedError(
            "Decoding diagonals back into the original grid is not supported."
        )
...
class ConnectedComponentRepresenter(GridRepresenter):
            ...
            for shape in contiguous_shapes:
                if self.normalized:
                    ...
                    if not self.disable_absolute:
                        shape_str = (
                            "[Abs. " + self.to_spreadsheet(min_i, min_j) + "]" + basic_shape_str
                        )
                    else:
                        shape_str = basic_shape_str
                else:
                    shape = [(i, j) for i, j in sorted(shape, key=lambda x: (int(x[0]), int(x[1])))]
                    shape_str = self.to_spreadsheet_with_dots(shape)
                shape_strings.append(shape_str)

            full_str = " | ".join(shape_strings)
            if self.max_token_per_color and self.max_token_per_color < len(full_str.split(" ")):
    ...
    def decode(self, encoded_str: str) -> Grid:
        ...
        max_row, max_col = 30, 30  # Adjusted for the given example, can be adjusted if needed
        grid = np.full((max_row, max_col), -1)  # Initialize with -1 to indicate empty cells
                ...
                for rel_position in rel_positions:
                    if "..." in rel_position:
                        ...
                        for row in range(start_row, end_row + 1):
                            grid[abs_row + row][abs_col + start_col : abs_col + end_col + 1] = color
                    else:
...
class CompositeRepresenter(GridRepresenter):
    ...
    def decode(self, encoded_str: str, actives: Optional[List[int]] = None) -> Grid:
        ...
        raise NotImplementedError("Decoding for CompositeRepresenter is not implemented.")


def build_text_grid_representer(include_spatial_views: bool = False) -> GridRepresenter:
    """Return the default textual grid formatter.
    ...
    base = PythonListGridRepresenter()
    if not include_spatial_views:
        return base

    rotated = RotatedGridRepresenter(
...
class ImageGridRepresenter(GridRepresenter):
    ...
    def decode(self, encoded_str: str, **kwargs) -> Grid:
        raise NotImplementedError("Decoding for ImageGridRepresenter is not implemented.")
    ...
    def encode(self, grid: Grid, connected_component: Optional[int] = None) -> str:
        ...
        for k, color in enumerate(sorted_index):
            ...
            for shape in contiguous_shapes:
                ...
                shape_str = f"Shape(color={color}, pos=({min_i},{min_j}), grid={subshape_str})"
                shape_strings.append(shape_str)
...
class TextExampleRepresenter(ExampleRepresenter):
    ...
    def decode(self, encoded: Tuple[str, str], **kwargs) -> Example:
        ...
        output_str = (
            output_str.replace(self.input_header, "").replace(self.output_header, "").strip()
        )
...
class ImageExampleRepresenter(ExampleRepresenter):
    ...
    def decode(self, encoded_str: str, **kwargs) -> Example:
        raise NotImplementedError("Decoding for ImageExampleRepresenter is not implemented.")
...
class DiffExampleRepresenter(ExampleRepresenter):
    ...
    def decode(self, encoded: Tuple[str, str], **kwargs) -> Example:
        ...
        output_str = (
            output_str.replace(self.diff_sep.strip(), "").replace(self.output_footer, "").strip()
        )
        input_grid = self.grid_representer.decode(input_str, **kwargs)
        if self.use_output:
...
class TextTaskRepresenter(TaskRepresenter):
    ...
    def decode(self, encoded: Tuple[str, str, str], **kwargs) -> Task:
        ...
        for example_str in train_str.split(
            ...
            train_example = self.example_representer.decode((input_str, output_str), **kwargs)
            train_examples.append(train_example)

        test_input_str = test.replace(self.test_header, "")
        test_example = self.example_representer.decode((test_input_str, encoded_output), **kwargs)
...
class ImageTaskRepresenter(TaskRepresenter):
    ...
    def __init__(self, example_representer: Optional[ImageExampleRepresenter] = None) -> None:
    ...
    def encode(self, task: Task, show_test_output=False, **kwargs) -> str:
        ...
        for k, example in enumerate(examples):
            ...
            grid = input_grid
            ax = axes[k, 0]
            ax.imshow(grid, cmap=self.example_representer.grid_representer.cmap, norm=norm)
            ax.set_title("Input")
            ax.grid(True, which="both", color="lightgrey", linewidth=0.5)
            ax.set_yticks([x - 0.5 for x in range(1 + len(grid))])
            ax.set_xticks([x - 0.5 for x in range(1 + len(grid[0]))])
